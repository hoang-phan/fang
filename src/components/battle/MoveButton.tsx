...
export function MoveButton({ move, playerMp, disabled, onClick }: MoveButtonProps) {
  ...
  if (!move) {
    return (
      <button
        disabled
        className="flex-1 min-w-0 rounded-lg border border-border-mid bg-theme-surface/50 p-2 lg:p-3 text-left opacity-30 cursor-not-allowed"
      >
  ...
  return (
    <button
      ...
      className={`
        flex-1 min-w-0 rounded-lg border p-2 lg:p-3 text-left transition-all duration-150
        ${isDisabled
